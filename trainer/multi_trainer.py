...
from utils.metrics import get_multi_confusion_matrix, get_multi_evaluation, get_confusion_matrix_plot
...
def each_train(dataset, homo_dataset, each_model, train_config):
    ...
    def inner_function(dataset, homo_dataset, each_model, train_config):
            ...
            if 'train' in train_config['history']['content']:
                train_columns = ['train_acc', 'train_f1', 'train_precision', 'train_recall']
                # train_fpr_columns = ['train_' + label + '_FPR' for label in train_config['labels']]
                # columns += train_columns + train_fpr_columns
                columns += train_columns
            
            if 'val' in train_config['history']['content']:
                val_columns = ['val_acc', 'val_f1', 'val_precision', 'val_recall']
                # val_fpr_columns = ['val_' + label + '_FPR' for label in train_config['labels']]
                # columns += val_columns + val_fpr_columns
                columns += val_columns
            
            if 'test' in train_config['history']['content']:
                test_columns = ['test_acc', 'test_f1', 'test_precision', 'test_recall']
                # test_fpr_columns = ['test_' + label + '_FPR' for label in train_config['labels']]
                # columns += test_columns + test_fpr_columns
            ...
            for epoch in range(1, train_config['epochs'] + 1):
                ...
                for data in train_loader:  # Iterate in batches over the training dataset.
                    data = data.to(device)
                    out = model(data.x, data.edge_index, data.batch) if model_config['type'] == 'homogeneous' else model(data.x_dict, data.edge_index_dict, data.batch_dict)
                    label_1 = data.y[0::2]
                    label_2 = data.y[1::2]
                    loss = criterion(out, label_2)  # Compute the loss.
                ...
                if 'train' in train_config['history']['content']:
                    ...
                    train_acc, train_f1, train_precision, train_recall, train_tb = get_multi_evaluation(train_y_target, train_y_pred)
                    # train_fpr = get_fpr(get_multi_confusion_matrix(train_y_target, train_y_pred)[0])
                    # logger.info(f'Train Acc: {train_acc:.4f}, Train F1: {train_f1:.4f}, Train Precision: {train_precision:.4f}, Train Recall: {train_recall:.4f}, Train FPR: {train_fpr}')
                    logger.info(f'Train Acc: {train_acc:.4f}, Train F1: {train_f1:.4f}, Train Precision: {train_precision:.4f}, Train Recall: {train_recall:.4f}')
                    # history_row += [train_acc, train_f1, train_precision, train_recall] + train_fpr
                    history_row += [train_acc, train_f1, train_precision, train_recall]
                ...
                if 'val' in train_config['history']['content']:
                    ...
                    val_acc, val_f1, val_precision, val_recall, val_tb = get_multi_evaluation(val_y_target, val_y_pred)
                    # val_fpr = get_fpr(get_multi_confusion_matrix(val_y_target, val_y_pred)[0])
                    # logger.info(f'Valid Acc: {val_acc:.4f}, Valid F1: {val_f1:.4f}, Valid Precision: {val_precision:.4f}, Valid Recall: {val_recall:.4f}, Valid FPR: {val_fpr}')
                    # history_row += [val_acc, val_f1, val_precision, val_recall] + val_fpr
                    logger.info(f'Valid Acc: {val_acc:.4f}, Valid F1: {val_f1:.4f}, Valid Precision: {val_precision:.4f}, Valid Recall: {val_recall:.4f}')
                    history_row += [val_acc, val_f1, val_precision, val_recall]
                ...
                if 'test' in train_config['history']['content']:
                    ...
                    test_acc, test_f1, test_precision, test_recall, test_tb = get_multi_evaluation(test_y_target, test_y_pred)
                    # test_fpr = get_fpr(get_multi_confusion_matrix(test_y_target, test_y_pred)[0])
                    # logger.info(f'Test  Acc: {test_acc:.4f}, Test  F1: {test_f1:.4f}, Test  Precision: {test_precision:.4f}, Test  Recall: {test_recall:.4f}, Test  FPR: {test_fpr}')
                    # history_row += [test_acc, test_f1, test_precision, test_recall] + test_fpr
                    logger.info(f'Test  Acc: {test_acc:.4f}, Test  F1: {test_f1:.4f}, Test  Precision: {test_precision:.4f}, Test  Recall: {test_recall:.4f}')
                    history_row += [test_acc, test_f1, test_precision, test_recall]
                ...
                if epoch in train_config['check_epoch']:
                    with torch.no_grad():
                        ...
                        # model = torch.load(os.path.join(train_config['save_dir'], f"{each_model}_{epoch}_model.pth"))
                        ...
                        cm_plt = get_confusion_matrix_plot(all_cm, train_config['labels'], normalize=False, path=os.path.join(train_config['logger']['root_dir'], f"{each_model}/{epoch}_confusion_matrix.pdf"))
...
def get_y(model, device, loader, type):
    ...
    for data in loader:
        ...
        y_target.extend(data.y[1::2].tolist())
